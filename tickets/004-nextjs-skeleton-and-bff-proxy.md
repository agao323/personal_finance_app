...
## Acceptance criteria
- [x] Route handler at `web/src/app/api/[...path]/route.ts` proxying to the internal API URL — forwards method, path, query, body, and status; strips hop-by-hop headers
- [x] `apiFetch` helper calling **relative** `/api/...` paths only. No absolute API origin anywhere in `web/`.
- [x] Root layout with Tailwind, fonts, and light/dark support
- [x] A page rendering `/ready` output, proving end-to-end connectivity through Docker
- [x] Vitest + Testing Library + MSW configured
- [x] Tests: unit for `apiFetch` URL building; functional for the proxy handler covering method, status, body, and error pass-through; component test for the page
